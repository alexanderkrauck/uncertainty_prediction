...
def get_dataset(name, base_path):
    ...
    if name=="facebook_1":
        df = pd.read_csv(base_path + 'Features_Variant_1.csv')        
        y = df.iloc[:,53].values
        X = df.iloc[:,0:53].values        
    
    if name=="facebook_2":
        df = pd.read_csv(base_path + 'Features_Variant_2.csv')        
        y = df.iloc[:,53].values
        X = df.iloc[:,0:53].values 
    ...
    if name=="community":
        ...
        data['OtherPerCap'] = data['OtherPerCap'].fillna(data['OtherPerCap'].astype(np.float32).mean())
