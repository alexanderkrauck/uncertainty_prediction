...
class Flow(ABC, nn.Module):
    ...
    def n_parameters(self) -> int:
        """
        :return: number of parameters in the flow
        """
        pass
...
class RadialFlow(Flow):
    ...
    def forward(self, x: Tensor, parameters: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        log_det_jac = (self.n_inputs - 1) * torch.log(1 + alpha * beta * divisor) + torch.log(1 + alpha * beta * divisor - alpha * beta * r * (divisor**2))
    ...
    def inverse(self, y: Tensor, parameters: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        x_approx = y + alpha * beta * divisor * difference
        log_det_jac = (self.n_inputs - 1) * torch.log(1 + alpha * beta * divisor) + torch.log(1 + alpha * beta * divisor - alpha * beta * r * (divisor**2))
...
class AffineFlow(Flow):
    ...
    def forward(self, x: Tensor, parameters: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        y = x * torch.exp(parameters[:, :self.n_inputs]) + parameters[:, self.n_inputs:]
    ...
    def inverse(self, y: Tensor, parameters: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        x = (y - parameters[:, self.n_inputs:]) * torch.exp(-parameters[:, :self.n_inputs])
        log_det_jacobian = -torch.sum(parameters[:, :self.n_inputs], dim=1)
...
class NFDensityEstimator(ConditionalDensityEstimator):
    ...
    def __init__(self, train_data_module, n_hidden: int, flows: List[str], dropout_rate=0.2, activation_function="relu",**kwargs):
        super().__init__()
        ...
        y_dim = train_data_module.train_dataset.y.shape[1]

        flow_list = []

        flow = [FLOW_MAP[flow](y_dim) for flow in flows]
        self.n_flows_params = nn.Parameter(torch.tensor([flow.n_parameters for flow in flow_list]), requires_grad=False)
        self.flows = nn.ModuleList(flow_list)
        self.n_flows = len(self.flows)

        n_output = sum(self.n_flows_params)

        self.mlp = MLP(n_inputs, n_hidden, n_output, dropout_rate, activation_function, **kwargs)
    ...
    def eval_output(self, y: Tensor, output: Tensor, normalised_output_domain: bool = True, reduce: str = "mean", **loss_hyperparameters) -> Tensor:
        
        y  = (y - self.mean_y) / self.std_y

        loss = self.nlll(y, **output, normalised_output_domain, reduce, **loss_hyperparameters)
    ...
    def get_density(self, x: Tensor, y: Tensor, normalised_output_domain: bool = False, numeric_stability: float = 1e-6, flow_params: Optional[Tuple[Tensor]] = None) -> Tensor:
        """
        Parameters
        ----------
        x : Tensor
    ...
    def nlll(self, y: Tensor, flow_params: Tuple[Tensor], normalised_output_domain: bool = True, reduce: str = "mean", **loss_hyperparameters) -> Tensor:
        """
        Parameters
        ----------
        y : Tensor
